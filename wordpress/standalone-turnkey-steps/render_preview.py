"""Render semzon-turnkey-steps.json to HTML using the template's own settings.
...
doc = json.load(open(os.path.join(OUT, "semzon-turnkey-steps.json"), encoding="utf-8"))
...
def render(node):
    ...
    if et == "column":
        ...
        # min-width:0 so a long word cannot push the card past its column -
        # Elementor's populated div is a block at 100% of a percentage-width
        # column, not a flex item that grows to fit its content.
        pop = ["position:relative", "display:flex", "flex-direction:column",
               "flex:1", "min-width:0", "box-sizing:border-box"] + surface(s)
    ...
    if wt == "divider":
        cls = responsive(node["id"], s)
        css = base + [f'width:{dim(s.get("width")) or "100%"}',
                      f'border-top:{dim(s.get("weight")) or "1px"} '
                      f'{s.get("style","solid")} {s.get("color","#000")}',
                      f'padding-block:{dim(s.get("gap")) or "15px"}',
                      "margin-inline:auto" if s.get("align") == "center" else ""]
        return f'<div class="{cls}" style="{";".join(c for c in css if c)}"></div>'

    if wt == "icon":
        cls = responsive(node["id"], s)
        if s.get("view", "default") == "default":
            sz = dim(s.get("size")) or "16px"
            css = base + [f"width:{sz}", f"height:{sz}", "border-radius:50%",
                          f'background:{s.get("primary_color","#000")}']
...
open(os.path.join(OUT, "preview-turnkey-steps.html"), "w", encoding="utf-8").write(page)
print("preview-turnkey-steps.html rendered from the template's own settings")
